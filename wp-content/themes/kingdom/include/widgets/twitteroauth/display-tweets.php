...
function get_auth($id, $max_tweets) {
    ...
    $cs_api_setting = get_option("cs_all_twitter_settings");
    if ($cs_api_setting <> "") {
        $xmlObject = new SimpleXMLElement($cs_api_setting);

        $consumer_key = isset($xmlObject->consumerkey) && $xmlObject->consumerkey <> '' ? (string) $xmlObject->consumerkey : '';
        $consumer_secret = isset($xmlObject->consumersecret) && $xmlObject->consumersecret <> '' ? (string) $xmlObject->consumersecret : '';
        $user_token = isset($xmlObject->accesstoken) && $xmlObject->accesstoken <> '' ? (string) $xmlObject->accesstoken : '';
        $user_secret = isset($xmlObject->accesstokensecret) && $xmlObject->accesstokensecret <> '' ? (string) $xmlObject->accesstokensecret : '';
    }

    require_once get_template_directory() . '/include/widgets/twitteroauth/includes/tmhOAuth.php';
...
function cache_json($id, $max_tweets, $time) {
    ...
    $cache = get_template_directory() . '/include/widgets/twitteroauth/cache/' . $id . '.json'; //Twitter cache directory
    $cache_folder = get_template_directory() . '/include/widgets/twitteroauth/cache/'; //Twitter cache directory
...
function display_tweets($id, $style = '', $max_tweets = 10, $max_cache_tweets = 10, $time = 60) {
    ...
    if (!empty($tweets)) {
        ...
        foreach ($tweets as $tweet) {
            ...
            $twitter .= "<div class= 'tweet'><h6 class='heading-color'>" . $tweet . "</h6>";
    ...
    echo '<div class="widget_slider"><div class="flexslider"><ul class="slides"><li>' . $twitter . '</li></ul></div></div>';
}

...
class CaptchaSecurityImages {
  ...
  function CaptchaSecurityImages($width = 145,$height = 35, $characters = 2 , $new_sess) {
    $code = $this->generateCode($characters);
    $font_size = $height * 0.60;
    $image = @imagecreate($width, $height) or die('Cannot initialize new GD image stream');

    /* set the colours */
    $bgR = mt_rand(0, 100); $bgG = mt_rand(0, 100); $bgB = mt_rand(0, 100);
    ...
    imagejpeg($image);
    imagedestroy($image);
    $_SESSION[$new_sess] = $code;
  }
}

$captcha = new CaptchaSecurityImages(145, 35, rand(4, 6), $_GET['new_sess'] );
